   ...
   selectNewStudent(event){
    ...
    render(){
        ...
        return (
            <div>
                <form>
                    <select onChange={this.selectNewStudent} name='selector'>
                        <option value={null}></option>
                        {students.map(student => <option value={student.id} key={student.id}>{`${student.firstName} ${student.lastName}`}</option>)}
                    </select>
                </form>
            <button onClick={() => enroll(this.state.studentId, schoolId)}>Enroll Student</button>
            </div>
        )
    }
...
const mapDispatchToProps = dispatch => {
    return {
        enroll: (studentId, schoolId) => dispatch(enrollStudent(studentId, schoolId))
    }
}

export default connect(null, mapDispatchToProps)(StudentEnroller)
